...
{	
    public static void Run(bool enableTestMode, DateTime testDateTime, DateTime dstStart, DateTime dstEnd, double dstOffset, double timeZone, double latitude, double longitude, out DateTime now, out bool isDst, out DateTime sunrise, out DateTime sunset, out bool isDark, out string about)
    {
		DateTime localTime = DateTime.UtcNow.AddHours(timeZone);		
		double offset;
		
		if(enableTestMode){
		 offset	= getDstOffset(dstStart, dstEnd, testDateTime, dstOffset);	
		 now = testDateTime;
		}else{
		 offset = getDstOffset(dstStart, dstEnd, localTime, dstOffset);		
		 now = localTime.AddHours(offset);		
		}
		
		isDst = offset != 0;
		sunrise = getSunrise(now, latitude, longitude, timeZone, isDst);
		sunset  = getSunset(now, latitude, longitude, timeZone, isDst);
		isDark = now < sunrise || now > sunset;
		
		about="Date: " + now.ToString("f");
		about += "\n" + "Time Zone: " + timeZone.ToString();
		about += "\nDaylight savings: " + isDst.ToString();
	...
	private static DateTime getSunrise(DateTime date, double latitude, double longitude, double timeZone, bool isDst){		
		double julianDate = Util.calcJD(date); 
		double sunRise = Util.calcSunRiseUTC(julianDate, latitude, longitude);
		return (DateTime) Util.getDateTime(sunRise, (int)timeZone, date, isDst);
	}
	
	private static DateTime getSunset(DateTime date, double latitude, double longitude, double timeZone, bool isDst){		
		double julianDate = Util.calcJD(date); 
		double sunSet = Util.calcSunSetUTC(julianDate, latitude, longitude);
		return (DateTime) Util.getDateTime(sunSet, (int)timeZone, date, isDst);
	}
	
	private static double getDstOffset(DateTime dstStart, DateTime dstEnd, DateTime now, double dstOffset){
		dstStart = ChangeYear(dstStart, now.Year);
		dstEnd = ChangeYear(dstEnd, now.Year);
		return (now > dstStart && now < dstEnd) ? dstOffset : 0;
	}
