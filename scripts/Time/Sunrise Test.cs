...
public static class Script1
{
	...
	static bool dst = false; // Day Light Savings 
	static DateTime date = DateTime.Today;
    
	public static void Run(out DateTime rise, out DateTime set)
    {
			...
			rise = (DateTime) Util.getDateTime(sunRise, zone, date, dst);
			set = (DateTime) Util.getDateTime(sunSet, zone, date, dst);
    }
}
